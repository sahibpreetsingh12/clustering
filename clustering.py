...
class Clustering:
    ...
    def get_distance(self,cen=None,arr=None,axis=1):

        """Returns the distance the centroid is from each data point in points."""
        return np.linalg.norm(cen - arr, axis=axis)
    ...
    def kmeans_calc(self,max_iter=50):
        ...
        for iter in range(max_iter):
            ...
            for i,cen in enumerate(centroids):
                distances[:,i]= self.get_distance(cen=cen,arr=arr,axis=1)
...
# For Kmeans refrence https://jonchar.net/notebooks/k-means/